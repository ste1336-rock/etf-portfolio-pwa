...
- **加權平均成本含買入手續費與稅**（v1.4 使用者裁決，取得成本認定）：成本基礎 = Σ(價×量 + 買入手續費 + 買入稅)；均價 = 成本基礎 / 總股數；未實現損益 = 市值 − 成本基礎。手續費與稅仍分別記錄於「累積手續費/稅」供透明呈現。賣出手續費與稅則依 §5.2 從已實現損益扣除。
...
## v1.4：買入手續費與稅併入成本（會計口徑調整，純前端）

- **成本基礎改為含買入手續費＋稅**（使用者裁決）：均價、未實現損益、已實現損益（賣出對應成本）都改用含取得成本的均價計算。更貼近國稅局/券商的成本認定。
- 手續費與稅仍在「累積手續費/稅」分別列示（買入部分已計入成本、賣出部分已扣損益），透明可查。
- 每筆交易的文字備註沿用既有「備註」欄（新增可填、點交易可改）。
- 稽核附帶修正（v1.3.1）：買入的稅先前被靜默丟失，已一併納入成本與統計。
...
- **累積手續費**：各幣別卡片顯示累積手續費/稅（v1.4 起買入部分已計入成本、賣出部分已扣損益，此行為透明呈現金額）。
